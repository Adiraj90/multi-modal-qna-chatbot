...
def setup_sidebar():
    ...
    with st.sidebar:
        ...
        <div style="text-align: center; padding: 0.5rem 0; margin-bottom: 0.5rem;">
            <h3 style="color: var(--accent-200); margin: 0; font-size: 1.6rem;">🧠 Context AI</h3>
...
def main():
    ...
    <div style="text-align: center; margin-bottom: 1rem;">
        <h1 style="font-size: 3rem; font-weight: 900; background: linear-gradient(90deg, var(--accent-200) 0%, var(--primary-300) 50%, var(--accent-100) 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 0.5rem; text-shadow: 0 10px 30px rgba(0, 0, 0, 0.3);">🧠 Context-Aware Chatbot</h1>
    ...
    <div class="context-info">
        ...
        <div style="color: var(--text-200); line-height: 1.6;">
            • Remembering last <strong style="color: var(--accent-200);">{context_length}</strong> messages<br>
            • Currently have <strong style="color: var(--accent-200);">{context_history_len}</strong> messages in memory
        </div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    if st.session_state.get("llm_instance"):
        ...
        with col3:
            st.markdown("""
            <div style="background: rgba(31, 43, 62, 0.4); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 20px; padding: 1.5rem; height: 100%; backdrop-filter: blur(10px);">
                <h4 style="color: var(--accent-200); margin-bottom: 0.8rem;">🔑 OpenAI</h4>
